...
const pageStyle = computed(() => ({
  backgroundImage: `linear-gradient(180deg, rgba(18, 24, 40, 0.22), rgba(31, 23, 16, 0.14)), url(${authPoster})`
}));
...
function resolveAuthError(e, action = "submit") {
  const code = e?.code;
  if (code === "A0401") {
...
async function sendRegisterSmsCode() {
  ...
  try {
    const data = await sendSmsCode(phone.value.trim(), "register");
    smsHint.value = `开发环境短信验证码固定为 ${data.mockCode || "123456"}`;
    startSmsCountdown(data.cooldownSeconds || 60);
  } catch (e) {
    error.value = resolveAuthError(e, "sendSms");
  } finally {
...
async function submit() {
  ...
  try {
    clearAuth();
    const data = await registerBySms(
      phone.value.trim(),
      smsCode.value.trim(),
      password.value,
      captchaId.value,
    ...
    setToken(data.token);
    setUser(buildUserSnapshot(data));
    router.push(normalizeRedirect(route.query.redirect));
  } catch (e) {
    clearAuth();
    error.value = resolveAuthError(e);
    ...
    <main class="auth-stage">
      <section class="auth-poster" :style="posterStyle">
        ...
        <div class="auth-poster-body">
          <p class="auth-kicker">手机号注册</p>
          <h1>建好账号之后，补药更快，问药更安心</h1>
          <p class="auth-description">拿到验证码就能建号，后面用密码或短信都能登录。</p>
        </div>

        <div class="auth-poster-footer">
          <span>注册后即可使用</span>
          <strong>短信登录 + 密码登录</strong>
          <small>开发环境短信验证码固定为 123456</small>
        </div>
      </section>

      <section class="auth-panel">
        <div class="auth-panel-head">
          <span class="auth-mini">用户注册</span>
          <h2>创建账号</h2>
          <p>手机号注册，拿到验证码就能完成。</p>
        </div>
          ...
          <div class="auth-field">
            ...
            <div class="auth-captcha-row auth-sms-row">
              <input
                ...
                placeholder="请输入验证码"
              />
              <button class="auth-secondary" :disabled="smsLoading || smsCountdown > 0" type="button" @click="sendRegisterSmsCode">
                {{ smsButtonText }}
              </button>
            </div>
